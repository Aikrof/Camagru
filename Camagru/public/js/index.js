...
var cou_block = document.getElementById('cou_block');
	...
	function sendRequestToGetContent()
	{
		...
		function creatResultDiv(result)
		{
			for (let val of result){
				let fst_div = document.createElement('div');
				...
				p.className = 'fst_p unselectable';
				p.innerHTML = val.login;
				...
				fst_div.appendChild(p);
				fst_div.appendChild(div);

				my_profile.appendChild(fst_div);
			}
	...
	window.addEventListener('click', (event)=>{
		...
		if (event.target.classList[0] === 'fst_p')
			userTransition(event.target.innerText);
	...
	window.addEventListener('scroll', (event)=>{
		...
		{
			if (window.pageYOffset + window.screen.height > my_profile.offsetHeight - 300)
			{
